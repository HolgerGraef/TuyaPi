  ...
  : QObject{ parent }
{
  connect(&mAgent,
          ...
          this,
          SLOT(onDeviceDiscovered(const QBluetoothDeviceInfo&)));
...
QStringList
BluetoothManager::devices() const
{
  QStringList result;
...
QString
BluetoothManager::iconString() const
{
  const auto& bt_con = fa::mdi6_bluetooth_connect;
  const auto& bt_notcon = fa::mdi6_bluetooth_off;
  return QString::fromStdU32String(std::u32string(
    { static_cast<char32_t>(mDevices.size() > 0 ? bt_con : bt_notcon) }));
}

void
BluetoothManager::onDeviceDiscovered(const QBluetoothDeviceInfo& dev)
{
