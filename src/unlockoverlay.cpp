...
void
UnlockOverlay::paintEvent(QPaintEvent* event)
{
  const auto& rect = QRectF((width() - mDiameter) / 2,
                            (height() - mDiameter) / 2,
                            mDiameter,
                            mDiameter);
...
unsigned
UnlockOverlay::progress()
{
  return 100 * (QDateTime::currentMSecsSinceEpoch() - mStartTimeMs) /
         mDurationMs;
}
